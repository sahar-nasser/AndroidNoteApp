    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                IconButton(onClick = {
                    viewModel.onEvent(NoteEvents.ToggleOrderSectionEvent)
                }) {
                    Icon(imageVector = Icons.Default.Sort, contentDescription = "Sort Notes")
                }
            }
            AnimatedVisibility(
                visible = state.isSectionVisible,
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(state.notes) { note ->
                    NoteItem(note = note, modifier = Modifier
                        ...
                        .clickable { },
                        onDeleteClick = {
                            viewModel.onEvent(NoteEvents.DeleteNoteEvent(note))
                            scope.launch {
                               val result =  scaffoldState.snackbarHostState.showSnackbar(
                                ...
                                if(result == SnackbarResult.ActionPerformed){
                                    viewModel.onEvent(NoteEvents.RestoreNoteEvent)
                                }
                            }
                        }
                    )
